...
defmodule Serverboards.Auth do
    ...
    user =
      case type do
        "basic" ->
          ...
          # requires update db
          |> ensure_exists_in_db(type) || email_auth(params)
  ...
  defp ensure_exists_in_db(user, creator) do
    ...
    nuser =
      case Serverboards.Auth.User.user_info(user.email) do
        {:error, :unknown_user} ->
    ...
    nuser =
      if nuser do
        # add extra perms
        nuser = %{nuser | perms: nuser.perms ++ Map.get(user, :perms, [])}
        ...
        if not MapSet.equal?(newgroups, oldgroups) do
          ...
          for g <- MapSet.difference(newgroups, oldgroups) do
            if g in group_list do
              ...
              :ok =
                Serverboards.Auth.Group.user_add(g, nuser.email, %{
                  email: creator,
                  perms: ["auth.manage_groups"]
                })
            end
          end
          ...
          for g <- MapSet.difference(oldgroups, newgroups) do
            ...
            :ok =
              Serverboards.Auth.Group.user_remove(g, nuser.email, %{
                email: creator,
                perms: ["auth.manage_groups"]
              })
          end
  ...
  defp try_login_by_auth(%{command: command, login: %{call: call}, id: id} = auth, params)
       when is_binary(command) and is_binary(call) do
    ...
    case Serverboards.Plugin.Runner.call(command, call, params, "system/auth") do
      {:ok, email} when is_binary(email) ->
        case Serverboards.Auth.User.user_info(email) do
          {:ok, user} ->
            Logger.info("Login via #{inspect(id)} for user #{inspect(email)}",
              user: email,
              auth: auth
            )
      ...
      {:ok, %{} = user} ->
        Logger.info("Login via #{inspect(id)} for user #{inspect(user["email"])}",
          user: user,
          auth: auth
        )
      ...
      [email] ->
        ...
        case Serverboards.Auth.User.user_info(email, [require_active: true], %{email: email}) do
          ...
          {:ok, me} ->
            ...
            base_url =
              Serverboards.Settings.get("serverboards.core.settings/base", %{})
  ...
  def auth_and_get_token(authdata) do
    ...
    try do
      ...
      any ->
        Logger.error(
          "Could not authenticate with params #{inspect(Map.drop(authdata, [:password]))}"
        )
