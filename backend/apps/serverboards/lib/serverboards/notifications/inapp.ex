...
defmodule Serverboards.Notifications.InApp do
  ...
  def notify(email, subject, body, meta) do
    ...
    user_id =
      Repo.one(from(u in Serverboards.Auth.Model.User, where: u.email == ^email, select: u.id))
    ...
    notification = %{
      user_id: user_id,
      subject: subject,
      body: first_not_empty([body, subject]),
      meta: meta,
      tags: tags
    }
    ...
    {:ok, notification} =
      Repo.insert(
        Model.Notification.changeset(
          %Model.Notification{},
          notification
        )
      )

    notification = %{notification | inserted_at: DateTime.to_iso8601(notification.inserted_at)}

    Serverboards.Event.emit("notifications.new", %{notification: notification}, %{
      user: email,
      perms: ["notifications.list"]
    })
  ...
  def update(id, what, me) do
    EventSourcing.dispatch(:notifications, :update, %{id: id} |> Map.merge(what), me.email)
  end
  ...
  def update_real(id, what, email) do
    ...
    model =
      %Serverboards.Notifications.Model.Notification{}
      |> Map.merge(details(id, me))
      |> Map.put(:user_id, me.id)

    {:ok, notification} = Repo.update(Model.Notification.changeset(model, what))
    # notification = %{ notification | inserted_at: DateTime.to_iso8601(notification.inserted_at)}
    Serverboards.Event.emit("notifications.update", %{notification: notification}, %{
      user: email,
      perms: ["notifications.list"]
    })
  end
  ...
  def list(filter, user) do
    ...
    q =
      Enum.reduce(Map.to_list(filter), q, fn
    ...
    ret =
      ...
      # post processing
      |> Enum.map(fn n ->
        %{n | inserted_at: DateTime.to_iso8601(n.inserted_at), body: String.slice(n.body, 0, 512)}
      end)
  ...
  def details(id, user) do
    ...
    # post process
    Map.merge(
      %{n | inserted_at: DateTime.to_iso8601(n.inserted_at)},
