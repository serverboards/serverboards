...
defmodule Serverboards.RulesV2.Rule do
  ...
  def init(rule) do
    ...
    rule = %{rule | rule: final_rule, from_template: nil}
  ...
  def decorate_params(params, desc) do
    Enum.reduce(desc, params, fn
      %{"name" => name, "type" => "service"}, params ->
        %{params | name => Serverboards.Service.decorate(params[name])}
  ...
  def start_trigger(uuid, w) do
    with [trigger] <- Serverboards.Rules.Trigger.find(id: w["trigger"]),
         {:ok, trigger_command} <-
           (if trigger.command do
              {:ok, trigger.command}
            else
              {:error, :no_trigger}
            end),
         {:ok, plugin_id} <- Serverboards.Plugin.Runner.start(trigger_command, "system/rule_v2"),
         {:ok, client} <- Serverboards.Plugin.Runner.client(plugin_id) do
      ...
      case Serverboards.Plugin.Runner.call(plugin_id, method, params) do
        ...
        {:error, error} ->
          Logger.error("Error starting rule #{inspect(uuid)}: #{inspect(error)}",
            rule_uuid: uuid,
            error: error
          )
  ...
  def handle_cast({:trigger, params}, state) do
    # already runing, no retriggers
    if state.running == false do
      {:ok, trigger_type} = Serverboards.Utils.map_get(state.rule.rule, ["when", "trigger"])

      Logger.info("Triggered #{inspect({state.rule.name, trigger_type})}",
        params: params,
        rule_uuid: state.rule.uuid
      )
      ...
      Logger.info("Ignoring retrigger of #{inspect(state.uuid)}. Already executing actions.",
        rule: state.rule
      )
  ...
  def handle_cast(:continue, %{running: []} = state) do
    ...
    state = %{state | running: false, signal_end_of_trigger: nil}

    # Run next in queue if any
    state =
      case state.trigger_queue do
        [] ->
          state

        [{from, params} | rest] ->
          GenServer.cast(self(), {:trigger, params})
          state = %{state | trigger_queue: rest, signal_end_of_trigger: from}
          state
      end
  ...
  def handle_cast(:continue, state) do
    ...
    state = %{state | running: head ++ rest, state: rule_state}
  ...
  def execute_action(
    ...
    if actiondef["debug"] do
      Logger.debug(
        "Run action #{inspect(action)} #{inspect(params)}\n #{inspect(state, pretty: true)}"
      )
    end
    ...
    result =
      if Enum.count(result) == 1 and Map.has_key?(result, :result) do
        result[:result]
      else
        result
      end
    ...
    state =
      if action_id do
        Map.put(state, action_id, result)
      else
        state
      end
  ...
  def execute_action(
    ...
    case eval_res do
      ...
      {:error, error} ->
        Logger.error("Error parsing condition #{inspect(condition)}: #{inspect(error)}.",
          rule_uuid: uuid
        )
  ...
  def execute_action(uuid, action, state) do
    Logger.warn("Unknown action or incomplete to execute #{action["type"]}",
      action: action,
      rule_uuid: uuid
    )
