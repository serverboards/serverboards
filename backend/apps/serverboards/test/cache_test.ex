...
defmodule Serverboards.CacheTest do
  ...
  test "Timeout is not ttl" do
    ...
    {time, _data} = :timer.tc(fn ->
      t1 = Task.async(fn -> Serverboards.Utils.Cache.get(test_id, &wait_1_sec/0, timeout: 1_000, ttl: 100) end)
      :timer.sleep(200)
      t2 = Task.async(fn -> Serverboards.Utils.Cache.get(test_id, &wait_1_sec/0, timeout: 1_000, ttl: 100) end)
