...
import sys, os
sys.path.append(os.path.join(os.path.dirname(__file__),'../../../../../../../plugins/bindings/python/'))
import serverboards
...
def schema(config, table=None):
    ...
    if table == "random":
        return {
            ...
            "description": "A table with a single random number. Can be tweaked to be between a max a min."
        }
