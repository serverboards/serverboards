...
DEBUG = False
# to be unique, not confuse with any response
PENDING = "66509317-642c-4670-a2ef-78155e32c4a9"
...
def printc(*s, color=None, hl=None, bg=None, **kwargs):
    ...
    if code:
        ...
        if bg:
            code += 10
        print("\033[{hl};{color}m{text}\033[1;m".format(
            hl=hl, text=text, color=code), **kwargs)
    else:
        print(text, **kwargs)
    sys.stdout.flush()
...
class IOClient:
    ...
    >>> client.call_line('@dir') # force remote call, if name collision
    ...
    def connect_to(self, fn):
        """
        Whenever this client receives a call from its own end, it will check
        the conditions at connections, and if True, calls the fn.

        fn(cmd, replyf)

        The fn must call the replyf only if it processes the request, and
        return True.
    ...
    def debug(self, *txt, **kwargs):
        if DEBUG or self._debug:
            ...
            printc("[%.2f] %s:" %
                   (time_since_start(),
                    self.__class__.__name__), *txt, **extra_args)

    def recv_and_parse(self):
        some = False
        for cmd in self.readcmds():
            # might be none to mark something was read, but no processing
            # required. Needed to know if should try again to select on fds.
            if cmd:
...
class CoreClient(IOClient):
        ...
        for cmd in self.readcmds_unfiltered():
            ...
            if 'result' in cmd or 'error' in cmd:
                ...
                if id in self.pending_replies:
                    ...
                    printc("Error, received not expeceted reply for message %s. Waiting for %s" % (
    ...
    def send(self, cmd, replyf):
        ...
        if '.' not in method and method not in self.METHOD_WHITELIST:
...
class CliClient(IOClient):
    ...
    def __init__(self, interactive=True, stdin=sys.stdin, stdout=sys.stdout):
        ...
        self.builtin = {
            ...
            'b64decode': lambda data, _id: base64.b64decode(
                data.encode('ascii')).decode('ascii'),
            'b64encode': lambda data, _id: base64.b64encode(
                data.encode('ascii')).decode('ascii'),
        }
        self.internal = {
    ...
    def builtin_call(self, cmd, replyf):
        ...
        if method in self.internal:
            ...
            if ret and self.interactive:
                printc("[%s]*: %s" % (cmd.get('id'), json.dumps(ret)),
                       color="purple", hl=True)
        return None
    ...
    def reply(self, reply):
        ...
        if 'result' in reply:
            ...
            if self.interactive:
                printc("[%s]: %s" % (id, json.dumps(
                    result, indent=2)), color="blue", hl=True)
            if result != self.vars:
                self.vars[""] = result
                ...
                if varname:
                    ...
                    if self.interactive:
                        printc("[%s]: %s" %
                               (varname, json.dumps(result, indent=2)))
            ...
            if id in self.pending_calls:
                ...
                if self.interactive:
                    printc("[%s]: %s" % (id, json.dumps(
                        reply["error"], indent=2)), color="red", hl=True)
        else:
    ...
    def internal_log(self, color, **kwargs):
        def real_log(msg, params={}, **kwargs):
            ...
            for m in msg.split('\n'):
                printc("%.2fs [%20s:%s] %s" %
                       (time_since_start(), file, line, m), color=color)
            return PASS
        return real_log
    ...
    def internal_dir_reply(self, count, baseid):
        self.internal_dir_store[baseid] = list(
            self.builtin.keys()) + list(self.internal.keys())

        def reply(cmd):
            nonlocal count
            count -= 1
            self.debug("More dir data:", cmd, count)
            if baseid not in self.internal_dir_store:
                return None
    ...
    def wait_for_reply(self, id, _timeout=1000):
        timeleft = _timeout
        while id not in self.last_replies and timeleft > 0:
            start_t = time.time()
    ...
    def cli_loop_select(self, timeout):
        ...
        """
        # requires thight loop, as it may be sending messages core<->cmd
        parsed_some = True
        while parsed_some:
            parsed_some = False
            self.debug("Checking if data ready: %s // to %s" %
                       (repr(self.filenos()), timeout))
            selres = select.select(self.filenos(), [], [], timeout)
            for n, clients_ready in enumerate(selres):
    ...
    def authenticate(self, auth_token=None):
        ...
        if auth_token:
            if self.call("auth.auth", type="token", token=auth_token):
                ...
                printc("Invalid (expired?) token. Authenticate again.",
                       color="red")
        for i in range(3):
            email = input("Email: ")
            password = getpass.getpass("Password: ")
            res = self.call(
                "auth.auth", type="basic", email=email, password=password)
            if res:
                settings["token"] = self.call("auth.token.create")
                try:
                    os.makedirs(os.path.expanduser(
                        "~/.config/serverboards/"), 0o700)
                except FileExistsError:
                    pass
                configfile = os.path.expanduser(
                    "~/.config/serverboards/s10s.json")
                with open(configfile, 'w') as wd:
...
class Completer:
    ...
    def __init__(self, client):
        ...
        histfile = os.path.join(os.path.expanduser(
            "~/.local/serverboards/"), "cmd_history")
        try:
    ...
    def complete(self, text, index):
        try:
            if index == 0:
                ...
                if text:
                    self.options = [
                        x + ' ' for x in self.options if x.startswith(text)]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Connect to Serverboards CORE or a command line plugin.')
    parser.add_argument('infiles', metavar='FILENAMES', type=str, nargs='*',
                        help='file to be parsed')
    parser.add_argument('--command', type=str,
                        help='Executes this command as a JSON-PC endpoint.' +
                             'To ease plugin debugging.')
    ...
    parser.add_argument('--no-auth', action='store_true',
                        help='Do not authenticate automatically. ' +
                        ' By default tries to use stored auth ' +
                        'token or asks for user/password')
