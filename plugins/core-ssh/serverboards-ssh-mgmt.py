...
def remote_fingerprint(url="", options="", **kwargs):
    ...
    sp=subprocess.Popen(["ssh-keygen", "-lf", "/dev/stdin", "-E", "sha1"], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    fingerprint_sha1, error = sp.communicate(fingerprintb)

    sp=subprocess.Popen(["ssh-keygen", "-lf", "/dev/stdin", "-E", "sha256"], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    fingerprint_sha256, error = sp.communicate(fingerprintb)

    fingerprint_text = ''.join( [str(fingerprint_md5, 'utf8'), "\n", str(fingerprint_sha1, 'utf8'), "\n", str(fingerprint_sha256, 'utf8')] )
...
def toggle_remote_fingerprint(url=None, status=None, options="", **args):
    ...
    if status["fingerprint_orig"]==fingerprint:
        ...
        if enabled:
            ...
            serverboards.info("Removed SSH known_host fingerprint for url %s"%(url))
            if done:
                return "Fingerprint removed"
            else:
                raise Exception("Could not remove fingerprint. Fingerprint not found. %s"%(hostname))
        else:
