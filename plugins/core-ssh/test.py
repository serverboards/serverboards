...
def t02_ssh_add_fingerprints_test():
    ...
    if os.path.isfile(authorized_keys_path):
        with open(authorized_keys_path, "r") as rd:
            if ssh_public_key in rd.read():
                serverboards.debug(
                    "SSH already in the users store to curent user keys," +
                    " will try to connect using ssh")
                exists = True
    if not exists:
        with open(authorized_keys_path, "a") as wd:
            serverboards.debug(
                "Adding the SSH key to curent user keys, " +
                "will try to connect using ssh")
            wd.write("%s\n" % ssh_public_key)
    ...
    if not remote_fingerprint["enabled"]:  # should have this key
        ok = mgmt.toggle_remote_fingerprint(
            url="localhost", status=remote_fingerprint)
...
def t03_ssh_scp_test_():
    ...
    try:
        ...
        pass
    assert ssh.scp(None, "/etc/hosts", service_uuid,
                   "/tmp/s10s-hosts-test")
...
def t04_ssh_backup_test_ignore():
    ...
    psoutput = subprocess.check_output(
        "ps aux | grep [o]f=/tmp/ || true", shell=True)
...
def t99_ssh_cleanup_fingerprint_test():
    ...
    mgmt = serverboards.Plugin("serverboards.core.ssh/mgmt")
    remote_fingerprint = mgmt.remote_fingerprint(url="localhost")
    assert mgmt.toggle_remote_fingerprint(
        url="localhost", status=remote_fingerprint) == "Fingerprint removed"
