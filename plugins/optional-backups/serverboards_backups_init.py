...
def program_backup(orig_days, time, id):
    ...
    timer_id = serverboards.rpc.add_timer( time_to_next, lambda: backup_now(orig_days, time, id) )
    timers[id] = timer_id

@serverboards.rpc_method
def backup_now(days, time, id):
    print("Do backup now of %s"%id)
    serverboards.action.trigger("serverboards.optional.backups/backup.now", dict(backup=id))
...
def maybe_recalculate_timers(plugin=None, key=None, value=None):
    ...
    for id, b in all_backups:
        if b["id"] == value["id"]:
            must_recalculate = (b["schedule"] != value["schedule"] or b.get("enabled") != value.get("enabled"))
    if must_recalculate:
