...
def printc(*s, color=None, hl=None, bg=None, **kwargs):
    ...
    if code:
        ...
        if bg:
            code += 10
        print("\033[{hl};{color}m{text}\033[1;m"
              .format(hl=hl, text=text, color=code), **kwargs)
    else:
        print(text, **kwargs)
    sys.stdout.flush()
...
class tmpdb:
    ...
    def __enter__(self):
        ...
        sh.psql(
            self.dbname,
            "-f", "backend/apps/serverboards/priv/repo/initial.sql",
            _out="log/create-tmpdb.txt"
        )
        databaseurl = ("postgresql://serverboards:serverboards@localhost/%s" %
                       self.dbname)
        with envset(MIX_ENV="test", SERVERBOARDS_DATABASE_URL=databaseurl), \
                chdir("backend"):
            sh.mix("run", "apps/serverboards/priv/repo/test_seeds.exs",
                   _out="../log/create-tmpdb.txt")
...
class running:
    ...
    def __enter__(self):
        ...
        self.process = cmd(*self.args, **self.kwargs,
                           _bg=True, _ok_code=range(16))
...
def random_dbname():
    return 'sbds_' + (
        ''.join(random.choice('abcdefghijklmnopqrst123467890')
                for _ in range(10))
    )
